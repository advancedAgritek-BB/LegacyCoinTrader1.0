...
2. Set up proper stop losses and take profits for all open positions that don't have them
...
def fix_existing_position_stops():
    """Set up stop losses and take profits for existing positions that don't have them."""
    ...
    if not config_path.exists():
        ...
        alt_paths = [
            ...
            Path(__file__).parent / "crypto_bot" / "config.yaml"
        ]
    ...
    print(f"Configuration: SL={stop_loss_pct*100}%, TP={take_profit_pct*100}%, TS={trailing_stop_pct*100}%")
    ...
    for position in positions:
        ...
        if position.stop_loss_price is not None and position.take_profit_price is not None:
            print(f"⚠️  {position.symbol}: Already has stop loss and take profit configured")
            continue
...
if __name__ == "__main__":
    try:
        updated = fix_existing_position_stops()
