...
monitoring_settings = monitoring_settings.clone(log_level=_execution_settings.log_level)
...
class ExecutionApplicationState:
        ...
        async with self._service_lock:
            ...
            if service is None:
                service = ExecutionService(
                    ExecutionServiceConfig.from_mapping(config or {})
                )
    ...
    async def register_order(
        ...
        ack_task = asyncio.create_task(
            self._capture_ack(service, client_order_id)
        )
        fill_task = asyncio.create_task(
            self._capture_fill(service, client_order_id)
        )
        async with self._order_lock:
    ...
    async def _capture_ack(
        self, service: ExecutionService, order_id: str
    ) -> None:
    ...
    async def aclose(self) -> None:
        ...
        for service in services:
            ...
            if callable(close):
                if asyncio.iscoroutinefunction(close):  # pragma: no branch
                    try:
                        await close()
                    except Exception:  # pragma: no cover - best effort cleanup
                        LOGGER.debug(
                            "Failed to close exchange cleanly", exc_info=True
                        )
                else:
                    try:
            ...
            if callable(ws_close):
                try:
                    ...
                    LOGGER.debug(
                        "Failed to close websocket client", exc_info=True
                    )
...
async def authorize_request(
    ...
    # Handle FieldInfo for service_token
    expected = getattr(settings, 'service_token', 'insecure-local-token-execution')
    if hasattr(expected, 'default'):
...
async def lifespan(app: FastAPI):
    ...
    # Ensure log_level is a string (handle FieldInfo issue)
...
def create_app() -> FastAPI:
    ...
    # Ensure base_path is a string (handle FieldInfo issue)
    base_path = getattr(settings, 'base_path', '/api/v1/execution')
    if hasattr(base_path, 'default'):
