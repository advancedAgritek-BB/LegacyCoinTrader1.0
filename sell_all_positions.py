...
This script uses the TradeManager as the source of truth and sells all
positions.
"""
...
def get_positions():
    ...
    try:
        response = requests.get('http://localhost:8080/api/open-positions')
...
def sell_position(symbol, amount):
    ...
    try:
        ...
        response = requests.post(
            'http://localhost:8080/api/sell-position',
...
def close_positions_in_trade_manager():
    ...
    try:
        from crypto_bot.utils.trade_manager import (
            get_trade_manager, create_trade
        )
        ...
        for pos in tm_positions:
            if pos.is_open:
                ...
                current_price = float(trade_manager.price_cache.get(
                ...
                print(f"Closed position {pos.symbol} in TradeManager "
                      "via closing trade")
                closed_count += 1
...
def main():
    ...
    for pos in positions:
        print(f"  {pos['symbol']}: {pos['amount']} ({pos['side']}) @ "
              f"${pos['current_price']:.4f}")

    # Auto-confirm for automated execution
    print("\nAuto-confirming: proceeding to sell all positions...")
    ...
    for position in positions:
        ...
        if result.get('success', False):
            ...
            print(f"✗ Failed to sell {position['symbol']}: "
