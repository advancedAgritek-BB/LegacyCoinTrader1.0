...
def validate_paper_wallet_consistency(ctx):
    ...
    try:
        ...
        if balance_diff > 0.01:
            logger.warning(f"Balance inconsistency detected: ctx.balance=${ctx.balance:.2f}, paper_wallet.balance=${ctx.paper_wallet.balance:.2f}, diff=${balance_diff:.2f}")
            return False
        ...
        if ctx_positions != wallet_positions:
            logger.warning(f"Position count mismatch: ctx.positions={ctx_positions}, paper_wallet.positions={wallet_positions}")
            return False
