...
class LatencyArbitrageEngine:
    ...
    def _find_cross_exchange_arb(self, symbol: str, prices: Dict[str, Dict]) -> Optional[ArbitrageOpportunity]:
        ...
        if profit_pct < self.config.min_profit_threshold:
            logger.debug(
                "%s cross-exchange profit %.4f below threshold %.4f for %s.",
                STRATEGY_NAME,
                profit_pct,
        ...
        logger.info(
            "%s detected cross-exchange arbitrage %s -> %s on %s (profit %.4f%%).",
            STRATEGY_NAME,
            buy_exchange,
            sell_exchange,
            symbol,
    ...
    def _find_triangular_arb(self, exchange: str, triangle: List[str],
        ...
        try:
            ...
            if best_profit < self.config.min_profit_threshold:
                logger.debug(
                    "%s triangular profit %.4f below threshold %.4f on %s.",
                    STRATEGY_NAME,
                    best_profit,
                    self.config.min_profit_threshold,
                    exchange,
            ...
            logger.info(
                "%s detected triangular arbitrage on %s via %s (profit %.4f%%).",
                STRATEGY_NAME,
                exchange,
                "->".join(triangle),
                best_profit * 100,
    ...
    def _find_latency_arb(self, exchange: str, symbol: str,
        ...
        try:
            ...
            if not bids or not asks:
                logger.debug(
                    "%s latency arb missing order book depth for %s on %s.",
                    STRATEGY_NAME,
                    symbol,
                    exchange,
                )
            ...
            if total_depth == 0:
                logger.debug(
                    "%s latency arb zero total depth for %s on %s.",
                    STRATEGY_NAME,
                    symbol,
                    exchange,
                )
            ...
            if abs(imbalance_ratio) > 0.7:  # 70% imbalance
                ...
                if profit_pct > self.config.min_profit_threshold:
                    ...
                    logger.info(
                        "%s latency arbitrage on %s %s imbalance %.2f (profit %.4f%%).",
                        STRATEGY_NAME,
                        exchange,
                        symbol,
                        imbalance_ratio,
                    ...
                    return opportunity
                logger.debug(
                    "%s latency arbitrage profit %.4f below threshold %.4f for %s %s.",
                    STRATEGY_NAME,
                    profit_pct,
                    self.config.min_profit_threshold,
                    exchange,
            ...
            try:
                if (self.opportunities_queue and
                    ...
                    if self.opportunities_queue:
                        logger.debug(
                            "%s concurrency limit reached (%d/%d active).",
                            STRATEGY_NAME,
    ...
    async def _execute_single_opportunity(self, opportunity: ArbitrageOpportunity) -> bool:
        ...
        try:
            ...
            if success:
                logger.info(
                    "%s executed %s arbitrage in %.1fms (net %.4f).",
                    STRATEGY_NAME,
                    opportunity.type,
                    execution_time,
                ...
                logger.warning(
                    "%s failed to execute %s arbitrage opportunity %s.",
                    STRATEGY_NAME,
                    opportunity.type,
                    opportunity.opportunity_id,
                )
