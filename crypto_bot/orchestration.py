...
from crypto_bot.metrics import (
    compute_average_atr,
    emit_cycle_timing,
...
async def execute_signals(ctx: BotContext) -> None:
    ...
    if not results:
        ...
        return
    # Prioritize by score
    telemetry_client = getattr(ctx, "telemetry", None)
    initial = len(results)
    results = [r for r in results if not r.get("skip") and r.get("direction") != "none"]
    filtered = initial - len(results)
    if filtered > 0 and telemetry_client is not None:
        telemetry_client.inc("exec.filtered_non_actionable", filtered)
    if not results:
        logger.info("PHASE: execute_signals - All signals filtered out - nothing actionable")
        if telemetry_client is not None:
            telemetry_client.inc("exec.no_actionable")
        return
    ...
    for candidate in results[:top_n]:
        ...
        if not ctx.position_guard or not ctx.position_guard.can_open(ctx.positions):
            ...
            if telemetry_client is not None:
                telemetry_client.inc("exec.blocked_max_open_trades")
            break
        sym = candidate["symbol"]
        if sym in ctx.positions:
            logger.info("Existing position for %s - skipping", sym)
            if telemetry_client is not None:
                telemetry_client.inc("exec.skip_existing_position")
            continue
        df = candidate["df"]
        ...
        if not allowed:
            ...
            if telemetry_client is not None:
                telemetry_client.inc(
                    f"exec.blocked_risk.{str(reason).strip().replace(' ', '_').lower()}"
                )
            continue
        ...
        if size <= 0:
            ...
            if telemetry_client is not None:
                telemetry_client.inc("exec.blocked_zero_size")
            continue
        if not ctx.risk_manager.can_allocate(strategy, size, ctx.balance):
            ...
            if telemetry_client is not None:
                telemetry_client.inc("exec.blocked_insufficient_capital")
            continue
        ...
        if side == "sell" and not ctx.config.get("allow_short", False):
            ...
            if telemetry_client is not None:
                telemetry_client.inc("exec.blocked_short_disabled")
            continue
...
async def _main_impl(
    *,
    ...
    if telemetry_client is None:
        from crypto_bot.utils.telemetry import telemetry as telemetry_client
    ...
    ctx.telemetry = telemetry_client
    ...
    try:
        while True:
            ...
            emit_cycle_timing(
                logger,
                symbol_t=timing.get("fetch_candidates", 0.0),
                ohlcv_t=timing.get("update_caches", 0.0),
                analyze_t=timing.get("analyse_batch", 0.0),
                total_t=total_time,
